...
const qNumbId = (qEl) => Number.parseInt(qEl.id.replace('q', ''));
const emptyQs = () => grid().filter(_qEl => _qEl.innerText === '');
const allSame = (arr) => arr.every(_qEl => _qEl.innerText === arr[0].ineerText && _qEl.innerText !== '');

const takeTurn = (index, letter) => grid()[index].innerText = letter;
const opponentChoice = () => qNumbId(emptyQs()[Math.floor(Math.random() * emptyQs().length)]);
...
const clickFn = ($event) => {
    takeTurn(qNumbId($event.target), 'x');
    opponentTurn();
};
